 ...
 * 
 * @author Marco Ly, Ahrenn Sivananthan, Tyler Zhang
 * @version Oct 2014
 */
public class Soldiers extends Humans
{
    ...
    /**
     * Adds the icon related to the soldier to the ZombieWorld when the soldier is created in ZombieWorld
     */
    public void addedToWorld(World ZombieWorld) {
        ZombieWorld.addObject(ammo, getX(), getY() -30);
    }
    ...
    public void act() 
    {
        ...
        if (numBullets <= 0 || outCounter >= 1000){
            ...
            returnToBase();            
            target = true;
        }else if (findTargets()){ //If the soldier is currently tracing a path to a target.                      
            shoot();
        }else{ //If the soldier does not see a target, wanders.
            wander();
            updateImg();
        }
    ...
    {
        //Have icon follow the soldier.
        if (numBullets > 0) {
            ammo.setLocation(getX(), getY() -30);
        }
    }
}